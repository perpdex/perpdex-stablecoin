...
export function createPerpdexExchangeFixture(
  ...
  return async (
    ...
    const perpdexExchange = (await perpdexExchangeFactory.deploy(
      settlementToken
    )) as TestPerpdexExchange;

    // priceFeed
    const priceFeed = await waffle.deployMockContract(
      owner,
      IPerpdexPriceFeedJson.abi
    );
